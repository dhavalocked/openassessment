...
import { transformItem as transformQti2 }           from '../../parsers/qti2/qti';
...
function getAnswerSelectedData(store, action) {
  ...
  const check = [transformQti2(item.xml)];
  const question = check[action.questionIndex];
  const answersById = {};
  _.each(question.answers, (answer) => {
    answersById[answer.id] = answer;
  });
  //console.log('debug: ', answersById);
